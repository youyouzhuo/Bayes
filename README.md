...
### 特征构建过程
高频词特征选择统计训练集词频Top-N的词语构建词袋向量，特征值直接采用词频计数，数学表达为f(w 
i)=count(w i )；TF-IDF特征则计算词项重要性权重，公式为f(wi)=tf(wi)×log(N/(df(wi)+1))，其中tf表示文档内词频，df为包含该词的文档数。实现差异体现在向量化工具选择：高频词模式采用CountVectorizer固定词表，TF-IDF模式使用TfidfVectorizer动态加权，前者计算效率高但忽略语义权重，后者增强特征区分度但增加计算复杂度。
### 特征模式切换方法
